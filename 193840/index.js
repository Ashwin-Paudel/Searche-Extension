                ...
                (()=>{
                    ...
                    solveCaptcha = (imageEl,outputEl,captchaIdentifier,callback=()=>{}
                        ...
                        if (window[captchaIdentifier] == undefined) {
                            ...
                            //solving takes some time, so we'll lock a duplicate solver instance out
                            ...
                            Tesseract.recognize(J(imageEl).attr("src")).then(equation=>{
                                ...
                                if (formula.length = 3) {
                                    //the exact length of the fomula
                                    if (formula.substr(1, 1) == 7) {
                                        //2-1 gets recognized as 271
                                        formula = formula.substr(0, 1) + "-" + formula.substr(2);
                                    }
                                    formula = formula.replace(/x/g, "*");
                                    //x is just the human version of *
                                    formula = formula.replace(/[} ]/g, "");
                                    //a random char being the result of misinterpretation; occasionally happening on the login form
                                    ...
                                    //not really necessary IF directly triggering a classic non-ajax post request
                    ...
                    function setTimerIntervalJS() {
                        timer(20000, // milliseconds
                        function(timeleft) {
                            // called every step to update the visible countdown
                            document.getElementById('timerTag').innerHTML = timeleft + " second(s)";
                        }, function() {
                            // what to do after
                    ...
                    setInterval(()=>{
                        ...
                        if (J("*:contains('503 Service Unavailable')").length) {
                            ...
                            if ((document.documentElement.textContent || document.documentElement.innerText).indexOf('Subscriber Limit Reached') > -1) {
                                ...
                                //This YouTube account no longer exists.
                                ...
                                //You took longer than 90 seconds to finish Subscribing to the account and confirming it.
                            ...
                            case "/login.php":
                                if (!J("#password").val().length)
                                    attachNotification("#username", "Consider storing your login data in your browser.");
                            ...
                            case "/youtubenew2.php":
                                ...
                                if (J(".followbutton").length) {
                                    //if false, there is likely a captcha waiting to be solved
                                    let vidID = ()=>{
                                    ...
                                    if (vidID() != previousVideo) {
                                        //has a new video has been provided yet? This will overcome slow network connections causing the same video to be played over and over
                                        previousVideo = vidID();
                            ...
                            GM.getValue("ylh_traffic_tab_open", false).then(state=>{
                                    ...
                                    if (J("*:contains('There are no Websites currently visitable for Points')").length) {
                                        ...
                                        if (buttons.length) {
                                            ...
                                            if (window.eval("typeof(window.childWindow) !== 'undefined'") && childWindow.closed)
                                                //without this we would not wait for the last link of the page to be visited successfully
                                                location.reload(true);
                                ...
                                case "/viewwebsite.php":
                                    ...
                                    setTimeout (window.close, 1000);*/
                                        ...
                                        alert("Please reload the website list, and make sure you are still logged in.");
                                    break;
                                }
                            }
                            );
